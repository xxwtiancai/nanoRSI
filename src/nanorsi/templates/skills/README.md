...
Set `[evaluator] counterfactual_enabled = true` before baseline to arm the gate's challenger: every candidate that clears the primary strict-improvement gate is re-scored together with its freshly measured parent on the manifest's ten counterfactual variants (renamed symbols, reordered lines, reworded instructions that preserve each family's rule) before admission, and a gain that vanishes there is labelled `shortcut`, consumes its attempt, never promotes, and enters the rejected-edit memory. Challenge evaluations are ordinary search episodes: arming the challenger adds ten parent and ten candidate episodes per gate-passing candidate on this template's panel.
