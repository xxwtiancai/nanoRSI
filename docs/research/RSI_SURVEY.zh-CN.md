...
本文是一部**活综述**：把 [catalog.json](industry-rsi/catalog.json) 中 147 条日期核验条目（滚动窗口 **2025-09-20 → 2026-09-20**，截至 **2026-09-20**）综合为一份分析文档，并随每日零点雷达扫查同步更新。下文每个论断都可回溯到资料库条目；作者结果**不等于**本地复现，本综述不宣称通用递归自改进已经实现。当前构成：**直接闭环（direct-loop）** 99 条、**支撑技术（enabling）** 37 条、**辅助研发（assisted-rd）** 11 条，横跨四个改变面——参数学习 40、智能体/代码 48、记忆/上下文 22、研究工作流 37——并进一步归入 **23 个机制家族**，分类页按家族分节呈现。
...
**5.1 验收闸门是全领域的收敛发明。** 82 条直接闭环条目中最强模式：持久化必须过独立检查。留出验证（SkillOpt、Experience Funnel）、验证器锚定提交闸门（SkillGLoW）、同条件配对比较（EvoOntology 无闸门 -11.2；EvoRS 匹配回放）、执行前审查（Apple）、新鲜会话审计（SkillEvolver）、处处可见的带日志回滚（WikiSkill 永不回滚的 wiki；Amazon 死胡同登记表）。昂贵闸门的廉价替代器是最新 refine——SkillLift 的秩相关重对齐评分表、Dream-RSI 的回放打分、Faraday 的自动 rubric。nanoRSI 的冻结评估器不变量正是该模式的架构化表述。
