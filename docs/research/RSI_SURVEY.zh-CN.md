...
本文是一部**活综述**：把 [catalog.json](industry-rsi/catalog.json) 中 135 条日期核验条目（滚动窗口 **2025-09-19 → 2026-09-19**，截至 **2026-09-19**）综合为一份分析文档，并随每日零点雷达扫查同步更新。下文每个论断都可回溯到资料库条目；作者结果**不等于**本地复现，本综述不宣称通用递归自改进已经实现。当前构成：**直接闭环（direct-loop）** 94 条、**支撑技术（enabling）** 31 条、**辅助研发（assisted-rd）** 10 条，横跨四个改变面——参数学习 38、智能体/代码 44、记忆/上下文 20、研究工作流 33——并进一步归入 **23 个机制家族**，分类页按家族分节呈现。
