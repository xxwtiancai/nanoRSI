...
The proposal context carries `rejected_recent`: bounded summaries (attempt id, decision, gate reason and hypothesis reason capped at 200 characters, changed paths) of the last five rejected, failed or no-op proposals. Metrics, diffs and artifacts never leak into the prompt. Fixture controls measure the plumbing delta: a memoryless scripted proposer repeats the identical rejected edit on every subsequent attempt, while a memory-aware one switches after the first rejection; the real-model effect on repeat-proposal rate is a separate live question.
