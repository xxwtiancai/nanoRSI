...
The reference Runner supports at most eight model actions per task. A model call whose action content fails strict JSON parsing is retried exactly once (both calls are recorded in usage and may produce `invalid_action`/`invalid_action_retry` markers), and a single outer markdown code fence is stripped before parsing; the payload inside remains strict JSON. `budget.max_steps` bounds all proposal attempts, not accepted generations. `max_episodes` bounds reserved search task executions; `max_output_bytes` caps captured raw bytes and result/proposal artifacts. UTF-8 replacement decoding may expand displayed bytes. Costs and token coverage are observations; absent values remain null, not zero. No dollar hard cap is advertised.

Candidate diffs are applied through a bounded git-native repair ladder — `git apply --recount`, then `--recount -C1` — which repairs miscounted and lightly drifted hunks typical of model-authored diffs while still rejecting hallucinated context. Diff path extraction falls back to `+++ b/` lines when no `diff --git` header is present.
...
The proposal context carries `rejected_recent` (disable per workspace with `[proposer] rejected_memory = false`): bounded summaries (attempt id, decision, gate reason and hypothesis reason capped at 200 characters, changed paths) of the last five rejected, failed or no-op proposals. Metrics, diffs and artifacts never leak into the prompt. Fixture controls measure the plumbing delta (memoryless scripted proposer 2/2 identical repeats vs memory-aware 0/2); the live A/B on the OpenEvolve function-minimization task found zero identical or near-identical repeats in either arm, so the memory stays on by default as a zero-observed-cost safeguard ([study](../examples/results/rejected-memory-ab/README.md)).
...
`nanorsi audit [--generation N]` writes `reports/audit.json` for the chosen accepted generation (default: latest). It checks every file on the **mutable surface** (all allow-patterned text files up to 64 KB at that generation's commit, reported with workspace-relative paths) for expected-answer leakage — whitespace-normalized task `expected_files` content of at least eight characters appearing in file text — and reports silent bypass: executable skills whose `run.py` never appears in a `skill_script_invoked` trace event across the generation's evaluation panel, plus declared-but-unloaded names. Markdown-only skills are always loaded into context and cannot be attribution-checked; they are reported separately instead of being guessed at. The audit is a read-only measurement: it feeds no gate, selection or contract, and a missing evaluation panel degrades to a leakage-plus-inventory report with an explicit note.

**Threat model for poisoned fixtures** (red-teamed in `tests/test_redteam.py`): the evaluator and task data are inside the trust boundary — an evaluator poisoned *before* baseline can hand a token candidate a passing score, and no in-loop control detects it; this is by design and the reason fixture review belongs before `baseline`. What the loop does guarantee: candidates cannot touch protected paths (surface policy), and any post-baseline change to the evaluator, tasks or configuration trips the frozen-contract guard, so poisoning can neither enter nor leave a started experiment silently. The audit's leakage scan covers the whole mutable surface, catching answers hidden outside the skills directory.
